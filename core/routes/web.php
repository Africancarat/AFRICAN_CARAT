...
// Compatibility route for legacy OmniMart image URLs.
// Many templates reference /core/public/storage/images/* while this app runs from project root.
Route::get('/core/public/storage/images/{path}', function ($path) {
    if (str_contains($path, '..')) {
        abort(404);
    }

    $fullPath = public_path('storage/images/' . $path);
...
Route::group(['middleware' => ['adminlocalize', 'demo']], function () {
    Route::prefix('admin')->group(function () { 
        ...
        Route::group(['middleware' => 'permissions:Manage Products'], function () {
            ...
            Route::get('/product/csv/sample', 'Back\CsvProductController@sample')->name('back.csv.sample');
        ...
        Route::group(['middleware' => 'permissions:Manage Site'], function () {
            ...
            Route::post('home-page/split-path-banner/update', 'Back\HomePageController@split_path_banner_update')->name('back.split.path.banner.update');
            ...
            Route::get('/setting/storage', 'Back\SettingController@storage')->name('back.setting.storage');
            Route::post('/setting/storage/link', 'Back\SettingController@storageLink')->name('back.setting.storage.link');
            //------------ EMAIL TEMPLATE ------------
...
Route::group(['middleware' => 'maintainance'], function () {
    Route::group(['middleware' => 'localize'], function () {
        ...
        Route::post('/review/sub', 'Front\FrontendController@slider_o_update')->name('front.rev.subbmit');
        ...
        //------------ DIAMONDS (4Cs facets, certificates, optical filters; isolated from OmniMart catalog) ------------
        Route::get('/diamonds', [App\Http\Controllers\Front\DiamondSearchController::class, 'index'])->name('diamonds.index');
        Route::get('/diamonds/compare', [App\Http\Controllers\Front\DiamondCompareController::class, 'index'])->name('diamonds.compare.index');
        Route::post('/diamonds/compare/add', [App\Http\Controllers\Front\DiamondCompareController::class, 'add'])->name('diamonds.compare.add');
        Route::get('/diamonds/compare/remove/{id}', [App\Http\Controllers\Front\DiamondCompareController::class, 'remove'])->name('diamonds.compare.remove');
        Route::post('/diamonds/compare/clear', [App\Http\Controllers\Front\DiamondCompareController::class, 'clear'])->name('diamonds.compare.clear');
        Route::post('/diamonds/inspection/request', [App\Http\Controllers\Front\DiamondInspectionController::class, 'requestSession'])->name('diamonds.inspection.request');

        //------------ EDUCATION ------------
        Route::prefix('education')->name('education.')->group(function () {
            Route::get('/', [App\Http\Controllers\Front\EducationController::class, 'index'])->name('index');
            Route::get('/trust-compliance', [App\Http\Controllers\Front\EducationController::class, 'compliance'])->name('compliance');
            Route::get('/interactive/4cs', [App\Http\Controllers\Front\EducationController::class, 'tool4cs'])->name('tool.4cs');
            Route::get('/guides/{slug}', [App\Http\Controllers\Front\EducationController::class, 'pillar'])
                ->whereIn('slug', \App\Support\EducationPillarRegistry::slugs())
                ->name('guides.show');
            Route::get('/{slug}', [App\Http\Controllers\Front\EducationController::class, 'show'])->name('show');
        });
