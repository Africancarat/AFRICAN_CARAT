...
class RazorpayController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        session(['razorpay_order_id'=> $razorpayOrderId]);
                    ...
                    $accent = (string) ($setting->primary_color ?? '#111111');
                    if ($accent !== '' && strpos($accent, '#') !== 0) {
                        $accent = '#'.$accent;
                    }
                    ...
                    $data = array_merge($data, FinancingHelper::razorpayMethodBlock());
	...
	public function notify( Request $request ) {
        ...
        if ($success)
        {
            $transaction_id = $_POST['razorpay_payment_id'];
                ...
                $billingExtras = Session::get('billing_address', []);
                ...
                $checkoutMeta = array_filter([
                    'occasion' => $billingExtras['checkout_occasion'] ?? ($requestData['checkout_occasion'] ?? null),
                    'important_date' => $billingExtras['important_date'] ?? ($requestData['important_date'] ?? null),
                    'delivery_estimate_snapshot' => $billingExtras['delivery_estimate_snapshot'] ?? ($requestData['delivery_estimate_snapshot'] ?? null),
                ], fn ($v) => $v !== null && $v !== '');
                if (! empty($billingExtras['concierge_requested']) || ! empty($requestData['concierge_requested'])) {
                    $checkoutMeta['concierge_requested'] = true;
                }
                if ($checkoutMeta !== []) {
                    $orderData['checkout_meta'] = $checkoutMeta;
                }
